...
function TierBadges({ user }) {
 ...
 <span className="text-[10px] font-semibold" style={{ color: cfg.color }}>{count}</span>
 </div>
 );
 })}
 </div>
...
function PodiumCard({ user, position, isMe }) {
 ...
 {isMe && (
 <div
 className="absolute -bottom-1 -right-1 text-[7px] font-semibold px-1.5 py-0.5 rounded-full"
...
function RankRow({ user, position, isMe, index }) {
 ...
 <span className="text-[12px] font-semibold" style={{ color: isMe ?'#8E9C78' :'#3a3a3c' }}>
 #{position}
 </span>
 )}
 </div>
 ...
 {}
 <span className="text-[13px] font-semibold text-zinc-400 shrink-0">{user.total}</span>
 </motion.div>
 );
}
...
export default function Leaderboard() {
 ...
 <h1 className="text-[38px] md:text-[44px] font-semibold text-white tracking-tighter mb-1 leading-none">
 Top Global
 </h1>
 ...
 <p className="text-[10px] font-semibold text-zinc-700 mb-2">
 Top 5
 </p>
 {top5.map((u, i) => (
 <RankRow
 ...
 Sua posição: <span className="font-semibold text-white">#{data.mePosition}</span>
 </p>
 </div>
 )}
 </>
