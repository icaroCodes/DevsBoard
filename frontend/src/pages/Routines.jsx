...
import { Plus, Trash2, Pencil, ChevronDown, CheckSquare, GripVertical, Repeat, X, Clock, Loader2, CircleDashed, Bell } from'lucide-react';
...
function SortableTask({ routineId, task, onToggle, onEdit, onDelete }) {
 ...
 <span className="px-1.5 py-0.5 rounded bg-white/5 text-[10px] text-[#86868B] font-bold tracking-tight">
...
function RoutinePreview({ r, visualLabels, isOverlay = false }) {
 ...
 <p className="font-semibold text-[14px] sm:text-[15px] text-[#F5F5F7] tracking-tight truncate flex-1 min-w-0 font-bold">{r.name}</p>
 </div>
...
function SortableRoutine({
 ...
 <div>
 <label className="block text-[11px] font-bold text-[#86868B] tracking-normal mb-1.5 ml-1">Em qual horário você começa?</label>
 <input
 type="time"
 ...
 <button
 type="button"
 onClick={() => setTaskForm({ ...taskForm, alarm_enabled: !taskForm.alarm_enabled })}
 className={`mt-2 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium transition-all border ${
 taskForm.alarm_enabled
 ?'bg-[#FFD60A]/10 border-[#FFD60A]/30 text-[#FFD60A]'
 :'bg-white/5 border-white/5 text-[#86868B] hover:text-[#F5F5F7]'
 }`}
 >
 <Bell size={11} />
 {taskForm.alarm_enabled ?'Alarme ativo' :'Ativar alarme'}
 </button>
 )}
 </div>
 {r.visual_type ==='weekly' && (
 <div className="col-span-1 sm:col-span-2">
 <label className="block text-[11px] font-bold text-[#86868B] tracking-normal mb-2.5 ml-1">Em qual dia da semana ocorre?</label>
 ...
 <label className="block sm:hidden text-[11px] font-bold text-[#86868B] tracking-normal mb-2 ml-1">Qual a importância disso?</label>
 ...
 className={`flex flex-1 sm:flex-none justify-center px-1 sm:px-3 py-2 sm:py-1.5 rounded-full text-[10px] sm:text-[11px] font-bold tracking-normal transition-all duration-200 ${taskForm.priority === p
...
export default function Routines() {
 ...
 const [taskForm, setTaskForm] = useState({ routineId: null, id: null, title:'', description:'', priority:'medium', start_time:'', day_of_week: null, alarm_enabled: false, submitting: false });
 ...
 success(editing ? t.routineUpdated : t.routineCreated);
 load();
 } catch (err) {
 error(err.message);
 ...
 alarm_enabled: taskForm.alarm_enabled && !!taskForm.start_time,
 };
 ...
 setTaskForm({ routineId: null, id: null, title:'', description:'', priority:'medium', start_time:'', day_of_week: null, alarm_enabled: false, submitting: false });
 ...
 {editing ? t.routineEditTitle : t.routineNewTitle}
 </h2>
 ...
 <div>
 <label className="block text-[13px] font-medium text-[#86868B] mb-2 tracking-normal">{t.routineNameLabel}</label>
 <input
 type="text"
 value={form.name}
 ...
 <div>
 <label className="block text-[13px] font-medium text-[#86868B] mb-3 tracking-normal">{t.routineFreqLabel}</label>
 ...
 ) : editing ? t.routineSaveEdits : t.routineCreateBtn}
 </button>
 </div>
 </form>
 </motion.div>
